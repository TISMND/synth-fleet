...
- **LED bar glow architecture:** `apply_led_bar()` applies a segment shader to the bar's background stylebox AND adds a child `led_glow` ColorRect with HDR color for bloom. The shader alone can't provide HDR bloom — Godot's 2D pipeline clamps canvas_item shader `COLOR` output, but preserves `ColorRect.color` values > 1.0. So the shader handles visual detail (segments, gaps, inner glow) while the glow rect provides the bloom source. Access shader via `bar.material`. The fill stylebox must be transparent (so ProgressBar value-scaling doesn't affect visuals); the background stylebox must be opaque (so the shader has a surface to draw on).
- **CanvasLayer content skips bloom.** WorldEnvironment glow post-processing does NOT apply to CanvasLayer content — it renders after the bloom pass. The HUD extends Control (not CanvasLayer) so bars participate in bloom. The VHS overlay uses its own CanvasLayer (layer 10) which is correct since it shouldn't bloom.
...
- **Bloom is resolution-dependent — PREVIEWS MUST MATCH GAME.** Godot's glow blur kernel is fixed-size in pixels. At each glow level (0=1/2, 1=1/4, ... 6=1/128 resolution), the blur covers a fixed pixel area, so it spans a proportionally *larger* fraction of small viewports vs large ones. A field that blooms beautifully in a 350px preview SubViewport will be nearly invisible at 1920×1080 with identical glow settings — and conversely, settings tuned for 1920px will look wildly overblown on smaller screens/devices. The whole point of a preview is to show what the player will actually see in-game. If a preview looks different from gameplay, the preview is lying and is worse than useless. **Current fix:** `VFXFactory.add_bloom_to_viewport()` compensates for large viewports (≥1024px) with three adjustments: (1) auto-enable glow levels 3–5 for wider blur spread, (2) lower `glow_hdr_threshold` by 0.25 so more pixels qualify, (3) scale `glow_intensity` by `√(width/400)`. **Future-proof approach:** If the compensation math drifts or breaks, the nuclear option is to render preview SubViewports at full game resolution (1920×1080) with content scaled up proportionally, then let `SubViewportContainer` with `stretch=true` downscale for display. This guarantees identical bloom because the glow pipeline runs at the same resolution. The preview will just show a scaled-down version of the actual game rendering. **Portability warning:** This resolution-dependence means bloom will blow out on smaller screens (tablets, phones, Steam Deck). Any future port MUST scale glow settings inversely with screen resolution, or all HDR-tuned effects will be massively overblown. The `add_bloom_to_viewport()` compensation already handles this directionally (only boosts for ≥1024px, leaves small viewports alone), but dedicated per-platform tuning may be needed.
