...
- **LED bars use HDR bloom:** `apply_led_bar()` applies a shader directly to the bar's background stylebox (no overlay child). Glow comes from HDR colors (>1.0) picked up by ThemeManager's global WorldEnvironment bloom. Access the shader via `bar.material` (not a child overlay).
