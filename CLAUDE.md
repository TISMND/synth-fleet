...
- **`apply_led_bar` inflates bar size:** The glow overlay child added by `ThemeManager.apply_led_bar()` uses anchor offsets (`offset_top = -pad_px`, etc.) that cause Godot to expand the bar's actual `size` well beyond `custom_minimum_size` (e.g. 128px → 320px). Setting `bar.size` and `bar.position` immediately after `apply_led_bar()` will not stick — Godot's layout recalculates. **Fix:** defer positioning with `call_deferred()`, read the bar's actual `size.y` at that point, and calculate position from the real dimensions.
