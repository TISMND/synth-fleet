extends Control
## Background Auditions — two tabs:
## 1. Shader Backgrounds: pure fragment-shader scrolling backgrounds
## 2. Experimental: hybrid and alternative methods for "handcrafted" feel
...
func _build_ui() -> void:
	...
	var margin := 20
	main_scroll.offset_left = margin
	main_scroll.offset_top = margin
	main_scroll.offset_right = -margin
	main_scroll.offset_bottom = -margin
	...
	_title_label.text = "BACKGROUND AUDITIONS"
	...
	main_vbox.add_child(_shader_content)
	...
	main_vbox.add_child(_experimental_content)
...
class _CityDoodads extends Node2D:
	...
	func _draw_water_tower(center: Vector2, radius: float) -> void:
		# Cylinder on legs: circle + support lines
...
class _CanvasCity extends Node2D:
			...
			for col in range(cols):
				...
				for py in range(subdiv_y):
					for px in range(subdiv_x):
						...
						# Merge adjacent plots for larger buildings sometimes
	...
	func _draw() -> void:
		...
		for bdata in _buildings:
			...
			# Building edge highlight (top-left = light, bottom-right = dark)
...
func _apply_theme() -> void:
	if _bg:
		ThemeManager.apply_grid_background(_bg)
	for btn in [_back_button, _tab_shaders_btn, _tab_experimental_btn]:
		if btn:
