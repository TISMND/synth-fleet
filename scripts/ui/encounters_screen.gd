extends Control
## Encounters screen — flight path editor.
## Left panel: path list. Center: bezier canvas. Right panel: properties.
...
func _build_toolbar() -> void:
	...
	bar.position = Vector2(LEFT_PANEL_W + 10, 6)
	bar.size = Vector2(vp_size.x - LEFT_PANEL_W - RIGHT_PANEL_W - 20, TOOLBAR_H)
...
func _build_left_panel() -> void:
	...
	var vp_size: Vector2 = get_viewport_rect().size
	panel.size = Vector2(LEFT_PANEL_W, vp_size.y)
...
func _build_canvas() -> void:
	_canvas = Control.new()
	var vp_size: Vector2 = get_viewport_rect().size
	var canvas_top: float = TOOLBAR_H + 12
	_canvas.position = Vector2(LEFT_PANEL_W, canvas_top)
	_canvas.size = Vector2(vp_size.x - LEFT_PANEL_W - RIGHT_PANEL_W, vp_size.y - canvas_top)
	_canvas.clip_contents = true
	_canvas.mouse_filter = Control.MOUSE_FILTER_STOP
	add_child(_canvas)

	# Override _draw and _gui_input via inner class
	...
	_canvas.add_child(drawer)

	# Replace _canvas reference with the drawable child for drawing/input
	_canvas = drawer

	_compute_canvas_rect()

	# Action buttons below canvas
	var btn_bar := HBoxContainer.new()
	btn_bar.position = Vector2(LEFT_PANEL_W + 10, vp_size.y - 44)
	btn_bar.size = Vector2(400, 36)
...
func _build_right_panel() -> void:
	...
	_right_panel.size = Vector2(RIGHT_PANEL_W, vp_size.y)
