...
func _build_ui() -> void:
	...
	_load_button.text = "(select weapon)"
...
func _open_icon_popup() -> void:
	...
	_icon_popup.setup_weapons(WeaponDataManager.load_all())
...
func _on_popup_selected(id: String) -> void:
	...
	_load_button.text = weapon.display_name
...
func _refresh_load_list() -> void:
	if _current_id != "":
		var weapon: WeaponData = WeaponDataManager.load_by_id(_current_id)
		if weapon:
			_load_button.text = weapon.display_name
		else:
...
func _populate_from_weapon(weapon: WeaponData) -> void:
	...
	_fade_in_slider.value = float(weapon.fade_in_ms)
	_fade_in_label.text = str(weapon.fade_in_ms) + "ms"
	_fade_out_slider.value = float(weapon.fade_out_ms)
	_fade_out_label.text = str(weapon.fade_out_ms) + "ms"
	...
	_populating = false
	_mark_clean()
	_update_effect_rate_label(_collect_weapon_data())
...
func _apply_bar_effects_once() -> void:
	...
	for bi in BAR_TYPES.size():
		...
		if slider and slider.value != 0.0:
			var effect_delta: float = slider.value * hits
			_stats_bar_values[bi] = clampf(_stats_bar_values[bi] + effect_delta, 0.0, _stats_bar_maxes[bi])
