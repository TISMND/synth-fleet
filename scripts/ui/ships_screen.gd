...
func _build_ally_cosmetic_tab(vbox: VBoxContainer) -> void:
	...
	pc_btn.color_changed.connect(func(c: Color) -> void:
		if _working_enemy:
			...
			if _paint_overlay and is_instance_valid(_paint_overlay):
				(_paint_overlay as ShipCosmetics.PaintOverlay).paint_color = c
	...
	lc_btn.color_changed.connect(func(c: Color) -> void:
		if _working_enemy:
			...
			if _light_overlay and is_instance_valid(_light_overlay):
				(_light_overlay as ShipCosmetics.LightOverlay).light_color = c
	)
...
func _update_cosmetic_overlays() -> void:
	...
	var overlays: Array[Node2D] = ShipCosmetics.build_overlays(
		_working_enemy.visual_id,
		_working_enemy.paint_pattern, _working_enemy.paint_color,
		_working_enemy.light_pattern, _working_enemy.light_color,
	)
	if overlays[0]:
