...
## Destinations auditions — 4 consolidated destination types with HDR glow sliders.
...
const DESTINATIONS: Array[Dictionary] = [
	...
	{"label": "METALLIC PLANET", "path": "res://assets/shaders/dest_metal_planet.gdshader", "hdr": 3.0},
]

const VP_SIZE := Vector2i(960, 480)
...
func _build_ui() -> void:
	...
	# 2x2 grid
	for row_idx in range(0, DESTINATIONS.size(), 2):
		...
		for col_idx in range(2):
			...
			if idx < DESTINATIONS.size():
				_build_cell(row, DESTINATIONS[idx])
