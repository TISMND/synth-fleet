...
static func save(id: String, data: Dictionary) -> void:
	...
	if not file:
		push_error("FieldEmitterDataManager: failed to save %s" % path)
		return
...
static func load_by_id(id: String) -> DeviceData:
	...
	if json.parse(file.get_as_text()) != OK:
		push_warning("FieldEmitterDataManager: JSON parse error in %s: %s" % [path, json.get_error_message()])
		return null
...
static func rename(old_id: String, new_id: String, data: Dictionary) -> void:
	if list_ids().has(new_id) and old_id != new_id:
		push_error("FieldEmitterDataManager: cannot rename to '%s' — ID already exists" % new_id)
		return
	if old_id == new_id:
		save(new_id, data)
		return
