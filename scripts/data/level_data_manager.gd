...
extends RefCounted
## Reads/writes level JSON files in res://data/levels/

const DIR_PATH := "res://data/levels/"
...
static func save(id: String, data: Dictionary) -> void:
	...
	if not file:
		push_error("LevelDataManager: failed to save %s" % path)
		return
...
static func load_by_id(id: String) -> LevelData:
	...
	if json.parse(file.get_as_text()) != OK:
		push_warning("LevelDataManager: JSON parse error in %s: %s" % [path, json.get_error_message()])
		return null
...
static func load_all() -> Array[LevelData]:
	...
	while fname != "":
		if fname.ends_with(".json"):
			var l: LevelData = load_by_id(fname.get_basename())
			if l:
				levels.append(l)
		fname = dir.get_next()
	dir.list_dir_end()
